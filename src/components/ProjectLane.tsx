...
interface ProjectLaneProps {
  ...
  onUpdateTask: (projectId: string, taskId: string, updatedTaskData: Partial<Omit<Task, 'id'>>) => void;
...
const ProjectLane: React.FC<ProjectLaneProps> = ({
  ...
  const getAssignedPeopleDetails = useMemo(() => {
    return (taskId: string) => {
        const task = project.tasks.find(t => t.id === taskId);
        if (!task) return [];
        return task.assignedPersons.map(personId => people.find(p => p.id === personId)).filter(Boolean) as Person[];
    }
  }, [project.tasks, people]);

  const handleReminderChange = async (taskId: string, taskTitle: string, e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const renderTaskItem = (task: Task, item: TaskItem, isCompletedList: boolean) => (
    <div key={item.id} className={`${styles.taskItem} ${isCompletedList ? styles.completedTaskItem : ''}`}>
      <input 
        type="checkbox" 
      ...
      {editingItem && editingItem.taskId === task.id && editingItem.itemId === item.id ? (
        ...
        <span 
          className={`${styles.taskItemText} ${item.completed ? styles.completedItemText : ''}`}
          onDoubleClick={() => handleItemTextDoubleClick(task, item)}
        >
          {item.text}
        </span>
      )}
      <button onClick={() => handleDeleteItem(task.id, item.id)} className={styles.deleteItemBtn}>×</button>
    </div>
  );
    ...
    <div className={styles.projectLaneContainer} style={{ borderColor: project.taskColor }}>
      <div className={styles.projectHeader}>
        {isEditingProjectTitle ? (
          ...
          <h2 onDoubleClick={handleProjectTitleDoubleClick} className={styles.projectTitle}>
            {project.title}
          </h2>
        )}
        <button onClick={() => onDeleteProject(project.id)} className={styles.deleteProjectBtn}>
          ✕
        </button>
      </div>
      ...
      <div className={styles.taskList}>
        {project.tasks.map(task => {
          ...
          const reminderValueForInput = task.reminder ? new Date(task.reminder).toISOString().substring(0, 16) : '';
          const currentTaskColor = task.color || project.taskColor; // Use task color if set, else project color

          return (
            <div key={task.id} className={styles.taskCard} style={{ backgroundColor: currentTaskColor }}>
              <div className={styles.taskHeader}>
                {editingTaskId === task.id ? (
                    ...
                    <p className={styles.taskTitle} onDoubleClick={() => handleTaskTitleDoubleClick(task)}>
                        {task.title}
                    </p>
                )}
                <button onClick={() => onDeleteTask(project.id, task.id)} className={styles.deleteTaskBtn}>
                    ✕
                </button>
              </div>
              ...
              <div className={styles.taskItemsContainer}>
                {task.items.filter(item => !item.completed).map(item => renderTaskItem(task, item, false))}
              </div>
              ...
              {task.items.some(i => i.completed) && (
                  <details className={styles.completedSectionDetails} open={false}>
                      ...
                      <div className={styles.completedItemsContainer}>
                          {task.items.filter(item => item.completed).map(item => renderTaskItem(task, item, true))}
                      </div>
                  </details>
              )}
              ...
              <div className={styles.addItemForm}>
                <input 
                  ...
                  onKeyDown={(e) => e.key === 'Enter' && handleAddNewItem(task.id)}
                  className={styles.addItemInput}
                />
                <button onClick={() => handleAddNewItem(task.id)} className={styles.addItemBtn}>+</button>
              </div>
              ...
              <div className={styles.taskFooter}>
                <div className={styles.assignedPeopleContainer}>
                  {assignedPeopleDetails.slice(0, 5).map(person => (
                    <span key={person.id} className={styles.personAvatar} title={person.name} onClick={() => handleRemovePersonFromTask(task.id, person.id)}>
                      {person.initials}
                    </span>
                  ))}
                  {assignedPeopleDetails.length > 5 && (
                    <span className={`${styles.personAvatar} ${styles.overflowAvatar}`}>+{assignedPeopleDetails.length - 5}</span>
                  )}
                  <button onClick={() => handleOpenPersonTagging(task.id)} className={styles.addPersonBtn}>+ Person</button>
                </div>
                ...
                {taggingPersonTaskId === task.id && (
                  <div className={styles.personTaggingPopup}>
                    <input 
                      ...
                      onKeyDown={(e) => e.key === 'Enter' && handleCreateAndAssignPerson(task.id)}
                    ...
                    {personSuggestions.length > 0 && (
                      <ul className={styles.personSuggestionsList}>
                        {personSuggestions.map(p => (
                          <li key={p.id} onClick={() => handleAssignPerson(task.id, p.id)}>
                            {p.name}
                          </li>
                        ))}
                      </ul>
                    )}
                    <button onClick={() => handleCreateAndAssignPerson(task.id)} className={styles.confirmAddPersonBtn}>Add/Assign</button>
                    <button onClick={() => setTaggingPersonTaskId(null)} className={styles.cancelAddPersonBtn}>Cancel</button>
                  </div>
                )}
                
                {/* Reminder Input */}
                <div className={styles.reminderContainer}>
                  <label htmlFor={`reminder-${task.id}`} className={styles.reminderLabel}>Reminder:</label>
                  <input 
                    type="datetime-local"
                    id={`reminder-${task.id}`}
                    value={reminderValueForInput}
                    onChange={(e) => handleReminderChange(task.id, task.title, e)}
                    className={styles.reminderInput}
                  />
                  {task.reminder && (
                    <button 
                        onClick={() => onUpdateTask(project.id, task.id, { reminder: undefined })}
                  ...
                  <div className={styles.colorPickerContainer}>
                    ...
                    {pickingColorForTask === task.id && (
                      <div className={styles.colorPalette}>
                        <button 
                           onClick={() => handleSetTaskColor(task.id, undefined)} 
                        ...
                        {PREDEFINED_TASK_COLORS.map(color => (
                          <button 
                            key={color}
                            onClick={() => handleSetTaskColor(task.id, color)}
        ...
        {project.tasks.length === 0 && <p className={styles.noTasksMessage}>No tasks yet. Add one below!</p>}
      </div>
      ...
      <div className={styles.addTaskForm}>
        ...
        />
        <button onClick={handleAddNewTask} className={styles.addTaskBtn}>+ Add Task</button>
      </div>

      <div className={styles.projectFooter}>
        <p>Color: <span style={{color: project.taskColor, fontWeight: 'bold'}}>{project.taskColor}</span> ({project.tasks.length} tasks)</p>
      </div>
    </div>
  );
};
